...
import { updatePairDayData, updateTokenDayData, updateMiuRamenDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let miuramen = MiuRamenFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  miuramen.totalLiquidityBNB = miuramen.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  miuramen.totalLiquidityBNB = miuramen.totalLiquidityBNB.plus(trackedLiquidityBNB);
  miuramen.totalLiquidityUSD = miuramen.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  miuramen.totalTransactions = miuramen.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let miuramen = MiuRamenFactory.load(FACTORY_ADDRESS);
  miuramen.totalVolumeUSD = miuramen.totalVolumeUSD.plus(trackedAmountUSD);
  miuramen.totalVolumeBNB = miuramen.totalVolumeBNB.plus(trackedAmountBNB);
  miuramen.untrackedVolumeUSD = miuramen.untrackedVolumeUSD.plus(derivedAmountUSD);
  miuramen.totalTransactions = miuramen.totalTransactions.plus(ONE_BI);
  ...
  let miuramenDayData = updateMiuRamenDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  miuramenDayData.dailyVolumeUSD = miuramenDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  miuramenDayData.dailyVolumeBNB = miuramenDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  miuramenDayData.dailyVolumeUntracked = miuramenDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  miuramenDayData.save();
